...
CREATE TABLE IF NOT EXISTS profile (
  ...
  goal TEXT DEFAULT 'maintain',      -- fat_loss | weight_loss | maintenance | lean_bulk | muscle_gain
  bmr REAL,
  tdee REAL,
  ...
  goal_weight_kg REAL,               -- target weight for the goal-date projection
  calorie_diff REAL,                 -- custom daily surplus/deficit chosen by the user (kcal)
  target_fiber REAL
);
...
CREATE TABLE IF NOT EXISTS walking_logs (
  ...
  terrain TEXT DEFAULT 'flat',       -- flat | slight_incline | steep_incline | trail | treadmill
  walk_type TEXT DEFAULT 'normal',   -- normal | brisk | power_walk | weighted_backpack
  speed_kmh REAL,
  met_used REAL,
  created_at TEXT DEFAULT (datetime('now'))
);
