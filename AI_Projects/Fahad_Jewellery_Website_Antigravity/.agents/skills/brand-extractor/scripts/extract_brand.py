#!/usr/bin/env python3
"""
Brand Extractor Script using Firecrawl v2 API.
Extracts brand identity, colors, typography, logos, hero images, and UI component styling,
and automatically generates a BRAND_GUIDELINES.md file.
"""
...
def fallback_extraction(target_url):
    """Fallback scraping if Firecrawl API is unavailable."""
    print("[Info] Running fallback brand extraction...", file=sys.stderr)
    try:
        ...
        logo_m = re.search(r'<img[^>]+src=["\']([^"\']*(?:logo|brand)[^"\']*)["\']', html, re.I)
        ...
        return {
            "branding": {
                ...
                "typography": {
                    "fontFamilies": {"primary": "Inter", "heading": "Playfair Display", "code": "Roboto Mono"},
                    "fontSizes": {"h1": "48px", "h2": "36px", "h3": "24px", "body": "16px"}
                },
                "spacing": {"baseUnit": 8, "borderRadius": "8px"},
                "components": {
                    "buttonPrimary": {"background": "#111827", "textColor": "#FFFFFF", "borderRadius": "8px"},
                    "buttonSecondary": {"background": "transparent", "textColor": "#111827", "borderColor": "#111827", "borderRadius": "8px"}
                },
                "images": {
                    "ogImage": og_img_m.group(1) if og_img_m else None,
                    "logo": logo_m.group(1) if logo_m else None
                }
            },
            "markdown": f"# Extracted content from {target_url}\n\nTitle: {title_m.group(1) if title_m else 'N/A'}"
        }
    except Exception as e:
        print(f"[Error] Fallback extraction failed: {e}", file=sys.stderr)
        return {}
...
def main():
    ...
    print(f"[*] Extracting brand data for: {args.url}")
    data = extract_brand_with_firecrawl(args.url, api_key)
    
    if not data:
        data = fallback_extraction(args.url)

    # Process asset downloads if requested
    downloaded_assets = {}
    if args.download_assets and data.get("branding"):
        asset_dir = os.path.join(".tmp", "brand_assets")
        images_info = data["branding"].get("images", {})
        logo_url = images_info.get("logo") or data["branding"].get("logo")
        og_url = images_info.get("ogImage")
        favicon_url = images_info.get("favicon")

        if logo_url:
            p = download_image(logo_url, asset_dir, "logo" + os.path.splitext(urllib.parse.urlparse(logo_url).path)[1] or ".png")
            if p: downloaded_assets["logo"] = p
        if og_url:
            p = download_image(og_url, asset_dir, "hero_og" + os.path.splitext(urllib.parse.urlparse(og_url).path)[1] or ".png")
            if p: downloaded_assets["hero_image"] = p
        if favicon_url:
            p = download_image(favicon_url, asset_dir, "favicon" + os.path.splitext(urllib.parse.urlparse(favicon_url).path)[1] or ".ico")
            if p: downloaded_assets["favicon"] = p

    data["downloaded_assets"] = downloaded_assets
